...
const textTemplate = `<h2>{{if .Email.Body.Title }}{{ .Email.Body.Title }}{{ else }}{{ .Email.Body.Greeting }} {{ .Email.Body.Name }},{{ end }}</h2>
{{ with .Email.Body.Intros }}
  {{ range $line := . }}
    <p>{{ $line }}</p>
  {{ end }}
...
type woningFinderTheme struct{}

// Name returns name
func (t *woningFinderTheme) Name() string {
	return "woningfinder"
}

// HTMLTemplate returns a Golang template that will generate an HTML email.
func (t *woningFinderTheme) HTMLTemplate() string {
	return htmlTemplate
}

// PlainTextTemplate returns a Golang template that will generate an plain text email.
func (t *woningFinderTheme) PlainTextTemplate() string {
	return textTemplate
}
