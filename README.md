...
- _[orchestrator](cmd/orchestrator)_, permits to orchestrate the different jobs that needs to be often ran by WoningFinder.
  ...
  - _ReminderEmail_ sends various reminder email to customers (such as missing corporation credentials). Runs everyday at 08:00, 16:00.
...
- _[city-location-updater](cmd/city-location-updater)_ permits to update the city location in the database. It is run as a job before every deploy.
